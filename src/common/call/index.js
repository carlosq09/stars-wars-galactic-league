...
const recursiveFetch = async (times, elementsArray = [], max = times, index = elementsArray.length || 1) => {
    return fetch(`https://swapi.dev/api/people/${index}/`)
        .then(response => {
            return response.json().then((data) => {
                elementsArray.push(data)
                if (index < times && index <= max) {
                    return recursiveFetch(times, elementsArray, max, ++index)
                } else {
